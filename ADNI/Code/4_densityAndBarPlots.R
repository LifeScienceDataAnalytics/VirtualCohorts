...
names(dfAll)[1] <- "value"
png(file = "/output/densityAll.png",
...
png(file = "/output/barPlotAll.png",
    width = 12, height = 8, units = 'in', res = 300)
ggplot(data = dfAllBar, aes(value,fill=Category, colour=Category)) +  facet_wrap(~ variable, scales = "free") +theme(strip.text.x = element_text(size=4)) + geom_histogram(position="dodge",stat="count")+theme_classic() + theme(axis.text = element_text(size=10),
...
library(infotheo)
mi <- sapply(real, function(u){
  sapply(simulated, function(v){
...
write.csv(mi, "/output/miADNI.csv")